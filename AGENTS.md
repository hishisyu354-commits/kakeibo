...
- **2つの単一HTMLアプリ**（依存なし・localStorageのみ）＋**1つの共用 Google Apps Script**（Gemini API 無料枠を利用）。
...
一人暮らしのユーザーが、**食費（1日1500円支給）の家計簿**と、**食材の賞味期限管理＋AI料理提案**を無料で回すための自作ツール群。iPhoneのSafariで「ホーム画面に追加」して使う。

- 💰 **食費家計簿**（`index.html`）… 使うたびに金額＋メモを追加。今日/日別/週別/月別の収支表示。カード利用通知メール→Apps Script 経由でほぼ自動同期。CSV取り込みもあり。
- 🧺 **食材ストッカー**（`pantry/index.html`）… 賞味期限管理・品切れ→買い物リスト・レシート撮影で在庫＋食費を同時記録・AI料理提案（詳細手順／絞り込み）・期限通知。
...
GitHub Pages (公開・同一オリジン)
  https://hishisyu354-commits.github.io/kakeibo/           -> index.html        (localStorage: "foodBudget.v1")
  https://hishisyu354-commits.github.io/kakeibo/pantry/     -> pantry/index.html (localStorage: "pantry.v1")
        |  ← 同一オリジンなので pantry から foodBudget.v1 に直接書ける（レシート機能で使用）
...
- **2アプリで同じ Apps Script URL を共用**（家計簿の「設定→自動同期」、ストッカーの「設定→AI連携」に同じ `.../exec?token=...` を貼る）。
...
```
index.html              食費家計簿（単一HTML, ~850行）
pantry/index.html       食材ストッカー（単一HTML, ~1200行）
apps-script/
...
- Pro解放コードの**平文**（配布はBOOTHのDL商品経由。git上は `pantry/index.html` の `PRO_HASH`＝SHA-256のみ）
...
- バックアップ書き出しに `apiUrl`（token付きURL）を含めない。インポート時は取り込んだ `apiUrl` を破棄し自端末の値を維持（悪意あるバックアップ対策）。写真はGeminiへ送るのでSETUP.mdで「食品以外を写さない」と案内済み。
...
- カード利用通知メール自動同期（`storage`イベントで pantry からの追記もクロスタブ反映＝add-onlyマージ）。

### 食材ストッカー（pantry/index.html）
- 在庫（賞味/消費期限・期限が近い順ソート `sortStock`・期限チップ色分け）。品切れ→買い物リスト（`markOut`/`restockSheet`）。
- **写真で追加**（`ai`）・**レシートで追加**（`receipt`）＝在庫＋食費家計簿(`foodBudget.v1`)に同時記録・**各在庫行の📷で期限だけ後から追加**（`scanExp`）。
...
このプロジェクトにテストランナーは無い。**Node で該当関数のソースを文字列抽出して素の関数として実行**するのが定石（DOM非依存の純ロジックのみ抽出。`el`/DOMに触れる関数は避け、`sanitize`/`daysLeft`/`sortStock`/`rakutenUrl`/`applyTheme` 等を検証）。パースチェックは:
```bash
node -e 'const fs=require("fs");const c=fs.readFileSync("pantry/index.html","utf8").match(/<script>([\s\S]*)<\/script>/)[1];new Function(c);console.log("parses OK")'
```
...
- [ ] **BOOTH出品URL**をユーザーからもらい `pantry/index.html` の `BOOTH_URL` を実URLに差し替え（現在は `https://booth.pm/` 仮）。DL商品にPro解放コードを同梱。
